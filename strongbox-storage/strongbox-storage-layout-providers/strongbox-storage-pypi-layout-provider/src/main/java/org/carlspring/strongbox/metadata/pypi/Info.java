...
public class Info
{
    public static final String[] CLASSIFIERS = {"Development Status :: 1 - Planning",
                                                 ...
                                                 "Topic :: Utilities",
                                                 "Typing :: Typed"};
    private static HashSet<String> classifiersHash;

    private static ReentrantLock HashLock = new ReentrantLock();
    ...
    private List<Map<String,Long>> downloads;
    ...
    /**
     * Checks if a string is a valid classifier according to the list at
    ...
    {
        if(classifiersHash == null)
        {
            HashLock.tryLock();
            if(HashLock.isHeldByCurrentThread()){
                try
                {
                    classifiersHash = new HashSet<>(CLASSIFIERS.length);
                    for (String s : CLASSIFIERS)
                    {
                        classifiersHash.add(s);
                    }
                }
                finally
                {
                ...
                //Wait until classifiersHash is setup
                while (classifiersHash == null || classifiersHash.isEmpty())
                {
                    try
                    {
                        Thread.sleep(10, 0);
